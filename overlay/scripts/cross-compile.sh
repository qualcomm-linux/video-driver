...
# Uses overlay scripts for cross-compilation
...
Cross-compile video driver using overlay scripts.
...
# Check if overlay scripts exist
if [ ! -f "$OVERLAY_DIR/scripts/set-build-env.sh" ]; then
    echo "Error: Overlay scripts not found. Please ensure overlay directory exists."
    exit 1
fi
...
# Call overlay script to setup environment
source "$OVERLAY_DIR/scripts/set-build-env.sh" "$COMPATIBLE"
