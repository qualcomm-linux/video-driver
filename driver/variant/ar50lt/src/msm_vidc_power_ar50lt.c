...
static int get_clock_corner_index(struct msm_vidc_core *core, u64 freq)
{
	...
	venus_hfi_for_each_clock(core, cl) {
		 ...
		 */
		if (freq && !strcmp(cl->name, "video_clk_src")) {
...
static int msm_vidc_calc_freq_ar50lt(struct msm_vidc_inst *inst, u32 data_size)
{
	...
	if (inst->domain == MSM_VIDC_ENCODER) {
		...
		vsp_cycles = mbs_per_second *  inst->capabilities[MB_CYCLES_VSP].value;

		/* 10 / 7 is overhead factor */
		vsp_cycles += (inst->capabilities[BIT_RATE].value * 10) / 7;
		...
		vsp_cycles = mbs_per_second * inst->capabilities[MB_CYCLES_VSP].value;
		/* 10 / 7 is overhead factor */
		vsp_cycles += div_u64((fps * clock_data->data_size * 8 * 10), 7);
...
int msm_vidc_scale_clocks_ar50lt(struct msm_vidc_inst *inst)
{
	...
	if (inst->power.buffer_counter < DCVS_WINDOW ||
		...
		inst->power.min_freq =
			get_clock_freq(core, "video_clk_src", get_max_clock_index(core));
		...
		msm_vidc_calc_freq_ar50lt(inst, clock_data->data_size);
