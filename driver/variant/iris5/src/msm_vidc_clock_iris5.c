...
static u32 codec_encoder_gop_complexity_table_fp_iris5[9][3];
static u32 input_bitrate_fp_iris5;
...
static u32 encoder_vpp_cycles_1pipe_iris5[3][8] = {
	/* h264e LCU16: 8KUHD, middle, UHD, middle, 1080p, middle, 720p, end */
 ...
 */
static u32 fp_pixel_count_bar1_iris5 = 3840 * 2160 * 240;
/* 1080p720 */
static u32 fp_pixel_count_bar2_iris5 = 3840 * 2160 * 180;
/* UHD120 */
static u32 fp_pixel_count_bar3_iris5 = 3840 * 2160 * 120;
/* UHD90 */
static u32 fp_pixel_count_bar4_iris5 = 3840 * 2160 * 90;
/* UHD60 */
static u32 fp_pixel_count_bar5_iris5 = 3840 * 2160 * 60;
/* UHD30; FHD120; HD240 */
static u32 fp_pixel_count_bar6_iris5 = 3840 * 2160 * 30;
/* FHD60 */
static u32 fp_pixel_count_bar7_iris5 = 1920 * 1080 * 60;
/* FHD30 */
static u32 fp_pixel_count_bar8_iris5 = 1920 * 1080 * 30;
/* HD30
 * static u32 fp_pixel_count_bar9_iris5 = 1280 * 720 * 30;
 */

/* Resolution to look up VPP Cycles: for Iris4 and future chipsets */
#define CODEC_RESOLUTION_720P_IRIS5                     (1280 * 720)
#define CODEC_RESOLUTION_1080P_IRIS5                    (1920 * 1080)
#define CODEC_RESOLUTION_3840x2160_IRIS5                (3840 * 2160)
...
static int initialize_encoder_complexity_table_iris5(void)
{
	...
	codec_encoder_gop_complexity_table_fp_iris5
...
static u32 get_bitrate_entry_iris5(u32 pixle_count)
{
	u32 bitrate_entry = 0;

	if (pixle_count >= fp_pixel_count_bar1_iris5)
		bitrate_entry = 1;
	else if (pixle_count >= fp_pixel_count_bar2_iris5)
		bitrate_entry = 2;
	else if (pixle_count >= fp_pixel_count_bar3_iris5)
		bitrate_entry = 3;
	else if (pixle_count >= fp_pixel_count_bar4_iris5)
		bitrate_entry = 4;
	else if (pixle_count >= fp_pixel_count_bar5_iris5)
		bitrate_entry = 5;
	else if (pixle_count >= fp_pixel_count_bar6_iris5)
		bitrate_entry = 6;
	else if (pixle_count >= fp_pixel_count_bar7_iris5)
		bitrate_entry = 7;
	else if (pixle_count >= fp_pixel_count_bar8_iris5)
		bitrate_entry = 8;
	else
		bitrate_entry = 9;
...
static u32 get_vpp_cycles_iris5(struct api_calculation_input codec_input)
{
	u8 i = 0, j = 0;
	u32 ret = 0;
	...
	if (codec_input.frame_width *
		...
		j = 0;
				...
				codec_input.frame_height > CODEC_RESOLUTION_4096x2160_IRIS5) {
		j = 1;
	} else if (codec_input.frame_width *
				codec_input.frame_height > CODEC_RESOLUTION_3840x2160_IRIS5) {
		if (codec_input.decoder_or_encoder == CODEC_DECODER)
			j = 2;
		else
			j = 1;
		...
		j = 2;
				...
				codec_input.frame_height > CODEC_RESOLUTION_1080P_IRIS5) {
		j = 3;
	} else if (codec_input.frame_width *
				codec_input.frame_height == CODEC_RESOLUTION_1080P_IRIS5) {
		j = 4;
	} else if (codec_input.frame_width *
				codec_input.frame_height > CODEC_RESOLUTION_720P_IRIS5) {
		j = 5;
	} else if (codec_input.frame_width *
				codec_input.frame_height == CODEC_RESOLUTION_720P_IRIS5) {
		j = 6;
...
static int calculate_vsp_min_freq_iris5(struct api_calculation_input codec_input,
...
static int calculate_apv_freq_iris5(struct api_calculation_input codec_input,
		struct api_calculation_freq_output *codec_output)
{
	u32 vsp_hw_min_frequency = 0;
	...
	if (codec_input.decoder_or_encoder == CODEC_DECODER) {
		...
		if (codec_input.video_adv_feature == 0) {
			if (codec_input.format_10bpp == 3)
				ppc = apv_encoder_ppc_iris5[ppc_index][0];
			else if (codec_input.format_10bpp <= 1)
				ppc = apv_encoder_ppc_iris5[ppc_index][2];
			else
				d_vpr_e("invalid format selected for APVe\n");
		} else if (codec_input.video_adv_feature == 1) {
			if (codec_input.format_10bpp == 3)
				ppc = apv_encoder_ppc_iris5[ppc_index][1];
			else if (codec_input.format_10bpp <= 1)
				ppc = apv_encoder_ppc_iris5[ppc_index][3];
			else
				d_vpr_e("invalid format selected for APVe\n");
		}
		vpp_hw_min_frequency =
	...
	if (codec_input.decoder_or_encoder == CODEC_DECODER) {
		tensilica_min_frequency =
			(DECODER_VPP_FW_OVERHEAD_APVD_IRIS5 *
			 codec_input.frame_rate * TENSILICA_CORE_RATIO_IRIS5 + 9) / 10;
		vpp_hw_min_frequency +=
			DECODER_VPP_FW_OVERHEAD_APVD_IRIS5 * codec_input.frame_rate;
		/* small resolution (<960*540) consideration */
		worker_Fmin = DECODER_WORKER_FW_OVERHEAD_NONAV1_IRIS5 *
						codec_input.frame_rate / 1000 / 1000;
	} else {
		tensilica_min_frequency =
			(ENCODER_VPP_FW_OVERHEAD_APVE_IRIS5 *
			 codec_input.frame_rate * TENSILICA_CORE_RATIO_IRIS5 + 9) / 10;
		vpp_hw_min_frequency +=
			ENCODER_VPP_FW_OVERHEAD_APVE_IRIS5 * codec_input.frame_rate;
		/* small resolution (<960*540) consideration */
		worker_Fmin =
			ENCODER_WORKER_FW_OVERHEAD_IRIS5 * codec_input.frame_rate / 1000 / 1000;
	}
	...
	fmin += sw_overhead_iris5[index] * codec_input.frame_rate;
...
static int calculate_vpp_min_freq_iris5(struct api_calculation_input codec_input,
		struct api_calculation_freq_output *codec_output)
{
	...
	u32 vpp_fw_overhead = DECODER_VPP_FW_OVERHEAD_NONAV1D_IRIS5;
	u32 vppvsp1stage_fw_overhead = DECODER_VPPVSP1STAGE_FW_OVERHEAD_NONAV1D_IRIS5;
	u32 worker_fw_overhead = DECODER_WORKER_FW_OVERHEAD_NONAV1_IRIS5;
	u32 vsp_hw_min_frequency = codec_output->vsp_min_freq * 1000 * 1000;
	u32 worker_Fmin = 0;
	u32 index = 0, temp = 0, len = 0;
	u32 vpp_target_clk_per_mb = get_vpp_cycles_iris5(codec_input);
	u32 hier_layer = 0;
	u32 gop_complexity = 0;


	if (codec_input.codec == CODEC_APV) {
		calculate_apv_freq_iris5(codec_input, codec_output);
		return 0;
	}
	...
	if (codec_input.decoder_or_encoder == CODEC_DECODER) { /* decoder */
		...
		vpp_fw_overhead = ENCODER_VPP_FW_OVERHEAD_IRIS5;
		...
		initialize_encoder_complexity_table_iris5();
		hier_layer = codec_input.hierachical_layer;
		gop_complexity =
		codec_encoder_gop_complexity_table_fp_iris5[hier_layer]
	...
	if (codec_input.vsp_vpp_mode == CODEC_VSPVPP_MODE_2S) {
		tensilica_min_frequency =
			(vpp_fw_overhead * codec_input.frame_rate *
			 TENSILICA_CORE_RATIO_IRIS5 + 9) / 10;
		...
		struct api_calculation_freq_output *codec_output)
{
	...
	rc = calculate_vsp_min_freq_iris5(codec_input, codec_output);
	if (rc)
		return rc;

	rc = calculate_vpp_min_freq_iris5(codec_input, codec_output);
	if (rc)
		return rc;
