...
#include "hfi_buffer_iris5.h"

static u32 msm_vidc_decoder_bin_size_iris5(struct msm_vidc_inst *inst)
{
	...
	if (inst->codec == MSM_VIDC_H264)
		HFI_BUFFER_BIN_H264D_IRIS5(size, width, height,
...
static u32 msm_vidc_decoder_comv_size_iris5(struct msm_vidc_inst *inst)
{
	u32 size = 0;
...
static u32 msm_vidc_decoder_non_comv_size_iris5(struct msm_vidc_inst *inst)
{
	u32 size = 0;
	u32 width, height, num_vpp_pipes;
	struct msm_vidc_core *core;
...
static u32 msm_vidc_decoder_line_size_iris5(struct msm_vidc_inst *inst)
{
	struct msm_vidc_core *core;
	u32 size = 0;
	...
	if (inst->codec == MSM_VIDC_H264) {
		if (inst->capabilities[SCALE_ENABLE].value) {
			HFI_BUFFER_LINE_H264D_DS_IRIS5(size, width, height,
					ds_width, ds_height, is_opb, num_vpp_pipes);
		} else {
			HFI_BUFFER_LINE_H264D_IRIS5(size, width, height,
						num_vpp_pipes);
		}
	} else if (inst->codec == MSM_VIDC_HEVC || inst->codec == MSM_VIDC_HEIC) {
		if (inst->capabilities[SCALE_ENABLE].value) {
			HFI_BUFFER_LINE_H265D_DS_IRIS5(size, width, height,
					ds_width, ds_height, is_opb, num_vpp_pipes);
		} else {
			HFI_BUFFER_LINE_H265D_IRIS5(size, width, height,
						num_vpp_pipes);
		}
	} else if (inst->codec == MSM_VIDC_VP9) {
		if (inst->capabilities[SCALE_ENABLE].value) {
...
static u32 msm_vidc_decoder_partial_data_size_iris5(struct msm_vidc_inst *inst)
{
	u32 size = 0;
	u32 width, height;
	struct v4l2_format *f;
...
static u32 msm_vidc_decoder_persist_size_iris5(struct msm_vidc_inst *inst)
{
	u32 size = 0;
	u32 rpu_enabled = 0;
	...
	if (inst->codec == MSM_VIDC_H264) {
		...
		HFI_BUFFER_PERSIST_VP9D_IRIS5(size);
	} else if (inst->codec == MSM_VIDC_AV1) {
		/*
		 * When DRAP is enabled, COMV buffer is part of PERSIST buffer and
...
static u32 msm_vidc_decoder_dpb_size_iris5(struct msm_vidc_inst *inst)
{
...
static u32 msm_vidc_encoder_bin_size_iris5(struct msm_vidc_inst *inst)
{
	struct msm_vidc_core *core;
	u32 size = 0;
...
static u32 msm_vidc_encoder_comv_size_iris5(struct msm_vidc_inst *inst)
{
	u32 size = 0;
...
static u32 msm_vidc_encoder_non_comv_size_iris5(struct msm_vidc_inst *inst)
{
	struct msm_vidc_core *core;
	u32 size = 0;
...
static u32 msm_vidc_encoder_line_size_iris5(struct msm_vidc_inst *inst)
{
	struct msm_vidc_core *core;
	u32 size = 0;
	...
	if (inst->codec == MSM_VIDC_H264)
		HFI_BUFFER_LINE_H264E_IRIS5(size, width, height, is_tenbit, num_vpp_pipes, 0,
						lookahead_enable);
	else if (inst->codec == MSM_VIDC_HEVC || inst->codec == MSM_VIDC_HEIC)
		HFI_BUFFER_LINE_H265E_IRIS5(size, width, height, is_tenbit, num_vpp_pipes, 0,
						lookahead_enable);
...
static u32 msm_vidc_encoder_dpb_size_iris5(struct msm_vidc_inst *inst)
{
	u32 size = 0;
...
static u32 msm_vidc_encoder_arp_size_iris5(struct msm_vidc_inst *inst)
{
	u32 size = 0;
...
static u32 msm_vidc_encoder_vpss_size_iris5(struct msm_vidc_inst *inst)
{
	u32 size = 0;
		...
		enum msm_vidc_buffer_type buffer_type)
{
	...
	static const struct msm_vidc_buf_type_handle dec_buf_type_handle[] = {
		...
		{MSM_VIDC_BUF_BIN,             msm_vidc_decoder_bin_size_iris5          },
		{MSM_VIDC_BUF_COMV,            msm_vidc_decoder_comv_size_iris5         },
		{MSM_VIDC_BUF_NON_COMV,        msm_vidc_decoder_non_comv_size_iris5     },
		{MSM_VIDC_BUF_LINE,            msm_vidc_decoder_line_size_iris5         },
		{MSM_VIDC_BUF_PERSIST,         msm_vidc_decoder_persist_size_iris5      },
		{MSM_VIDC_BUF_DPB,             msm_vidc_decoder_dpb_size_iris5          },
		{MSM_VIDC_BUF_PARTIAL_DATA,    msm_vidc_decoder_partial_data_size_iris5 },
	};
	static const struct msm_vidc_buf_type_handle enc_buf_type_handle[] = {
		{MSM_VIDC_BUF_INPUT,           msm_vidc_encoder_input_size              },
		{MSM_VIDC_BUF_OUTPUT,          msm_vidc_encoder_output_size_iris5       },
		{MSM_VIDC_BUF_INPUT_META,      msm_vidc_encoder_input_meta_size         },
		{MSM_VIDC_BUF_OUTPUT_META,     msm_vidc_encoder_output_meta_size        },
		{MSM_VIDC_BUF_BIN,             msm_vidc_encoder_bin_size_iris5          },
		{MSM_VIDC_BUF_COMV,            msm_vidc_encoder_comv_size_iris5         },
		{MSM_VIDC_BUF_NON_COMV,        msm_vidc_encoder_non_comv_size_iris5     },
		{MSM_VIDC_BUF_LINE,            msm_vidc_encoder_line_size_iris5         },
		{MSM_VIDC_BUF_DPB,             msm_vidc_encoder_dpb_size_iris5          },
		{MSM_VIDC_BUF_ARP,             msm_vidc_encoder_arp_size_iris5          },
		{MSM_VIDC_BUF_VPSS,            msm_vidc_encoder_vpss_size_iris5         },
	};
...
static int msm_vidc_input_min_count_iris5(struct msm_vidc_inst *inst)
{
...
static int msm_buffer_delivery_mode_based_min_count_iris5(struct msm_vidc_inst *inst,
	uint32_t count)
{
	struct v4l2_format *f;
		...
		enum msm_vidc_buffer_type buffer_type)
{
	...
	case MSM_VIDC_BUF_INPUT_META:
		count = msm_vidc_input_min_count_iris5(inst);
		break;
