...
# Uses DKMS-Debian scripts for cross-compilation

set -e

SCRIPT_DIR="$(dirname "$0")"
DKMS_DEBIAN_DIR="$SCRIPT_DIR/DKMS-Debian"
...
Cross-compile video driver using DKMS-Debian scripts.
...
# Check if DKMS-Debian scripts exist
if [ ! -f "$DKMS_DEBIAN_DIR/scripts/set-build-env.sh" ]; then
    echo "Error: DKMS-Debian scripts not found. Please ensure DKMS-Debian directory exists."
    exit 1
fi
...
# Call DKMS-Debian script to setup environment
